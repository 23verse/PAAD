...
- We reinforce the prognostic value of our identified genes in DNA repair and RTK related signaling pathways by analyzing PAAD patients from the TCGA cohort.

- We suggest the **[treatment management]** for PAAD patients: repair-deficient and proliferation-active subtypes are better suited for DNA damage therapies, while immunotherapy highly recommended for repair-proficient and repair-enhanced subtypes.
...
```{r train, fig.cap="", fig.width=9.8, fig.height=3, echo=FALSE, eval=T, fig.align="center",out.width="100%"}
library(png)
library(grid)
img <- readPNG("booklet.PAAD.train.png")
grid.raster(img)
```

We attempted to construct a prognostic model that is clinically actionable using the CNV information of genes involved in DNA repair and RTK related signaling pathways: 
...
```{r test, fig.cap="", fig.width=3.3, fig.height=3, echo=FALSE, eval=T, fig.align="center",out.width="50%"}
library(png)
library(grid)
img <- readPNG("booklet.PAAD.test.png")
grid.raster(img)
```
